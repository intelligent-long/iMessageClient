...
public class ServerMessageServiceStomp {
    ...
    private static synchronized void subscribe(Context context) {
        ...
        stomp.subscribe(StompDestinations.GROUP_CHANNEL_NOTIFICATIONS_UPDATE, null, message -> {
            ServerMessageServiceStompActions.updateGroupChannelNotifications(context);
        });
        ...
        stomp.subscribe(StompDestinations.GROUP_CHANNEL_COLLECTIONS_UPDATE, null, message -> {
            ServerMessageServiceStompActions.updateGroupChannelCollections(context);
        });
    }
